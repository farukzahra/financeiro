...
import { isSystemBudgetOrigem } from "../services/credit-card-budget.js";
...
export async function registerBudgetRoutes(app: FastifyInstance) {
  ...
  app.patch<{ Params: { id: string } }>("/budget/:id", async (req, reply) => {
    ...
    const [current] = await db
      ...
      .limit(1);
    if (!current) return reply.code(404).send({ error: "Nao encontrado" });
    if (isSystemBudgetOrigem(current.origem)) {
      return reply.code(403).send({ error: "Item de sistema não pode ser editado" });
    }
  ...
  app.delete<{ Params: { id: string } }>("/budget/:id", async (req, reply) => {
    ...
    if (!current) return reply.code(404).send({ error: "Nao encontrado" });
    if (isSystemBudgetOrigem(current.origem)) {
      return reply.code(403).send({ error: "Item de sistema não pode ser excluído" });
    }
    await db
      .delete(budgetItems)
      .where(and(eq(budgetItems.userId, user.id), eq(budgetItems.id, req.params.id)));
